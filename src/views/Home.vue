<template>
  ...
  <el-carousel height="4.4rem">
    <el-carousel-item v-for="(item,index) in arr" :key="index" :class="'index'+index">
      <div class="container">
        <div class="left">
          <img :src="item.img">
        </div>
        <div class="right">
          <div>
            ...
            <h3 v-for="(iten,inde) in item.text.context" :key="inde+'a'">{{iten}}</h3>
          </div>
          <img v-show="item.littleImg" :src="item.littleImg" alt="" class="littleImg">
          <a :href="item.download">
            <el-button type="success" icon="el-icon-download">立即下载</el-button>
          </a>
        </div>
      </div>
    </el-carousel-item>
...
export default {
  ...
  setup(){
    ...
    const updateLog=[{
      title: '2021/07/02 07:14 (1). 病毒库升级到2021.07.02.07:14，新增539个威胁特征。本日第1次更新，日累计新增539。总数：10147187'
    }]
    return {
      arr,
      fnData,
